...
ap.add_argument(
    ...
    "--query",
    required=False,
    help="tab-seperated query file containing UniProt IDs",
)
ap.add_argument(
    "-n",
    "--nterm",
    required=False,
    help="N-terminal topology. Must be either 'in' or 'out'",
)
...
def fetch_structures():
    ...
    if args.crop:
        for entry_id, tm in zip(
            query["Entry"].to_list(), query["Transmembrane"].to_list()
        ):
                ...
                try:
                    with open(structfile, "w") as outfile:
                        for line in r.text.split("\n"):
                            if (not "END" in line) and (
                                is_desired_tm_portion(line, tm_start, tm_end)
                            ):
                                outfile.write(line + "\n")
...
def run_ppm(local_structures):
    ...
    subprocess.run(["bash", "run_ppm.sh"], cwd=os.getcwd())
...
def analyse_structures():
    ...
    for i in result_structures:
        analysis = zres.Zres(i).run(buffer=40)
